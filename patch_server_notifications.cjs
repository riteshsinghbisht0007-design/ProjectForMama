...
code = code.replace(mongoInitCode, newMongoInitCode);
  ...
  app.get('/api/notifications', requireAuth, async (req: any, res: any) => {
    ...
    try {
      ...
      for (const summon of summons) {
        ...
        // Normalize time to compare just the dates (since both are parsed as UTC midnight for YYYY-MM-DD)
      ...
      // 3. Delete outdated notifications (e.g. if hearing date changed, the old uniqueKeys remain)
      // Actually, it's safer to just fetch and return, we can optionally clean up old ones later or let them be marked as read.
